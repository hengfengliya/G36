...
当前版本使用 **Vercel Serverless Functions 的临时存储（/tmp目录）**，这意味着：
...
**Serverless Functions（无服务器函数）**的工作原理：
- 不是传统的24小时运行的服务器
- 按需启动，像"临时工"一样
- 有请求时启动函数，处理完就进入待机状态
- 长时间无请求就被回收，下次请求重新启动
...
**举例说明：**
```
8:00  - 用户A访问 → 启动函数实例#1 → 创建空数据
8:05  - 用户B访问 → 使用实例#1 → 可以看到用户A的数据
8:10  - 用户C访问 → 使用实例#1 → 可以看到A和B的数据
8:30  - 15分钟无人访问 → Vercel回收实例#1
8:35  - 用户D访问 → 启动新实例#2 → 数据重置为空！
```
...
**Q: 为什么我的数据过一会儿就没了？**
A: 因为使用的是临时存储（/tmp目录），Serverless Functions闲置15分钟左右会被回收，数据会重置。这是免费方案的限制。

**Q: 是否支持多房间？**
A: 当前版本所有用户共享一个房间。如需多房间功能，建议部署多个实例或升级到数据库方案。

**Q: 如何实现真正的数据持久化？**
A: 需要集成数据库服务（如Vercel KV、Supabase等）。可以查看项目的Issues或提交需求。

**Q: 数据会不会被其他人看到？**
A: 会的。当前所有访问同一部署地址的用户共享数据（在同一函数实例内）。

**Q: 如何修改队伍人数限制？**
A: 编辑 `index.html` 中的 `MAX_PLAYERS` 常量即可。

**Q: 可以本地运行吗？**
A: 可以！克隆项目后运行 `npm run dev`，本地运行时数据会在当前会话中保持。
